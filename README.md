...
2 - Dada a atribuição dos dados aos clusters, minimizamos com respeito a $\textbf{c}_i$, ou seja, atualizamos as posições dos clusters e consequentemente ```math \dfrac{\partial O}{\partial \textbf{c}_j} = \displaystyle{\sum_{n=1}^{N}} z_{n,j}(\textbf{x}_n-\textbf{c}_j)=0\nonumber```,
